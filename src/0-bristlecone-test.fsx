...
let hypothesis =
    { Equations  = [ code "x", vonBertalanffy ] |> Map.ofList
      ...
      Parameters = [ code "eta",    parameter PositiveOnly   0.001 1.00
                     ...
                     code "kappa",  parameter PositiveOnly   0.001 1.00 ] |> Map.ofList }
...
let method =
      ...
      |> Bristlecone.withCustomOptimisation (Optimisation.MonteCarlo.SimulatedAnnealing.fastSimulatedAnnealing 0.10)
...
let addNoise p data =
      ...
      let draw = Bristlecone.Statistics.Distributions.Normal.draw random 0. (sigma ** 2.)
      data
      |> Map.map(fun key value -> TimeSeries.map value (fun (x,y) -> x + draw(), y))
