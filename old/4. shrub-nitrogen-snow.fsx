...
module ModelComponents =
    ...
    module Proxies =
        ...
        let shrubHeightCm radiusMM =
            radiusMM / 10. |> Allometrics.shrubHeight Constants.k5 Constants.k6
...
let ``base model`` maxGrowthRate nLimitation nitrogenFeedback nReplenishment protectionEffect temperatureDependency additionalParameters =
    ...
    let dbsdt' (b:float) n gammab r maxGrowthRate limit protectionEffect tempEffect =
        match limit with
        | Some l -> ModelComponents.BaseEquations.biomass b n r gammab maxGrowthRate l protectionEffect tempEffect
        | None -> ModelComponents.BaseEquations.biomass b n r gammab maxGrowthRate (fun _ -> 1.) protectionEffect tempEffect

    /// Bioavailable soil nitrogen [N]
    let dndt' bs n gamman maxGrowthRate feedback limit nReplenishment tempEffect = 
        match limit with
        | Some l -> ModelComponents.BaseEquations.soilNitrogen n bs gamman nReplenishment maxGrowthRate l feedback tempEffect
        | None -> ModelComponents.BaseEquations.soilNitrogenNoUptake n bs gamman nReplenishment feedback
