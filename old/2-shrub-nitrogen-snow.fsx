...
module Options =
    ...
    let orchestrator = OrchestrationAgent(logger, 2, false)
...
let hypotheses =
    ...
    let snowProtection =
        [ (fun p e -> ModelComponents.SnowProtection.none), []
          (fun p e -> ModelComponents.SnowProtection.linearSnowProtection (p |> Pool.getEstimate "spe") (lookup e "bs" |> ModelComponents.Proxies.shrubHeightCm) (26. - (lookup e "d18O"))),
        ...
        [ (fun p -> ModelComponents.GrowthLimitation.hollingDiscModel ((p |> Pool.getEstimate "a") / 1000.) ((p |> Pool.getEstimate "r") * 1000.) (p |> Pool.getEstimate "h")),
          ...
          (fun p -> ModelComponents.GrowthLimitation.linear ((p |> Pool.getEstimate "a") / 1000.)), 
           [ code "a",      parameter PositiveOnly   0.001 0.010
             code "r",      parameter PositiveOnly   2.000 8.000 ] ]       // N-uptake efficiency
          //(fun _ -> ModelComponents.GrowthLimitation.none), 
          // [ code "r",       parameter PositiveOnly   2.000 8.000 ] ]
