...
let ``nutrient-dependent growth`` =
    ...
    { Equations  = [ ShortCode.create "x",      drwdt
      ...
      Parameters = [ // for growth function
                     ShortCode.create "fMax",   Parameter.create PositiveOnly   0.200 1.600   // Maximum rate of resource-saturated growth per unit biomass
                     ...
                     ShortCode.create "gamman", Parameter.create PositiveOnly   0.300 1.500   // Loss rate of nitrogen
                     // for shrub allocation and physiology
                     ShortCode.create "r",      Parameter.create PositiveOnly   0.100 0.400   // Respiration cost per unit biomass
                     ShortCode.create "q",      Parameter.create PositiveOnly   0.001 0.200  // Nutrient requirement per unit biomass
                     ShortCode.create "gammab", Parameter.create PositiveOnly   0.001 0.750  // Loss rate of biomass
                     // for likelihood function
                     ShortCode.create "rho",    Parameter.create Unconstrained  -0.90 0.90   // Covariance between growth and nitrogen
                     ShortCode.create "sigmax", Parameter.create PositiveOnly   1.000 4.000   // Standard deviation of x (biomass)
...
R.plot( namedParams [ "x", box (estimated.[0].[0].Trace |> List.map fst |> List.rev |> List.skip 1000) ; "type", box "l"; "xlab", box "Iteration"; "ylab", box "-log likelihood" ]) |> ignore
...
module Proxies =
    ...
    /// Radius in millimetres
    let toBiomassMM stems radiusMM = 
        radiusMM / 10. |> ModelComponents.Allometrics.shrubBiomass Constants.b Constants.a Constants.rtip Constants.p Constants.lmin Constants.k5 Constants.k6 stems Constants.salixWoodDensity
...
let r,b,s =
    ...
    |> List.collect(fun stems ->
        [ 1. .. 50. ]
        |> List.map(fun r -> r, Proxies.toBiomassMM (float stems) r, float stems ) )
    |> List.unzip3
...
open RProvider.scales

R.ggplot(df, R.aes__string(namedParams [ "x", box "StemRadius"; "y", box "Biomass"; "group", box "StemCount"; "color", box "StemCount" ])) 
++ R.geom__line()
++ R.theme__classic()
++ R.labs(namedParams [ "x", "Stem Basal Radius (mm)"; "y", "Shrub Stem Biomass (g)"])
