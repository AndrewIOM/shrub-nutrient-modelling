...
module Options =
    let resultsDirectory = "/Users/andrewmartin/Desktop/Bristlecone Results/LongTermWithRepeatedNValues-TunedMCMC/"
    let chains = 3
    let endWhen = Optimisation.EndConditions.afterIteration 250000
    let logger = Logging.Console.logger() //Logging.RealTimeTrace.graphWithConsole 15. 10000
    let engine =
        Bristlecone.mkContinuous 
        |> Bristlecone.withContinuousTime Integration.MathNet.integrate
        |> Bristlecone.withOutput logger
        |> Bristlecone.withTunedMCMC [ Optimisation.MonteCarlo.TuneMethod.CovarianceWithScale 0.250, 500, Optimisation.EndConditions.afterIteration 250000 ]
        // |> Bristlecone.withCustomOptimisation (Optimisation.MonteCarlo.Filzbach.filzbach 0.01 (Optimisation.EndConditions.afterIteration 50000))
        // |> Bristlecone.withTunedMCMC [ Optimisation.MonteCarlo.TuneMethod.CovarianceWithScale 0.250, 500, Optimisation.EndConditions.afterIteration 150000 ]
...
let bivariateGaussian' (p:ParameterPool) obsx obsy expx expy = 
    ...
    let q = (1. / (1. - rho ** 2.)) * (zta1 + zta3 - zta2) //+ zta4)
    vNegLog + (1./2.) * q
...
let ``base model`` maxGrowthRate nLimitation nitrogenFeedback additionalParameters =
    ...
    { Equations  = [ code "bs",        dbsdt
      ...
      Parameters = [ // for nitrogen dynamics
                     ...
                     code "gamma[b]",  parameter PositiveOnly   0.0001 0.001   // Loss rate of biomass
...
let workPackages shrubs hypotheses engine saveDirectory =
    seq {
        for s in shrubs do
            ...
            // Shrub should be constrained to above 1.53mm, INCLUDING conditioning point.
            ...
            for h in [ 1 .. hypotheses |> List.length ] do
                for _ in [ 1 .. Options.chains ] do
                    yield async {
                            ...
                            // B. Save to file
                            Bristlecone.Data.EstimationResult.saveAll saveDirectory s.Identifier.Value h 10 result
                            return result }
    }
...
//work |> Seq.rev |> Seq.iter (OrchestrationMessage.StartWorkPackage >> Options.orchestrator.Post)
...
type Slope = {
    PValue: float
    Slope: LinearRegressionCoefficient
    Intercept: LinearRegressionCoefficient
}

let regression (x:float[]) y =
    let x' = x |> Array.map (fun a -> [|a|])
    let mlr = MultipleLinearRegressionAnalysis(true)
    let _ = mlr.Learn(x', y)
    { PValue = (mlr.Coefficients |> Seq.head).TTest.PValue; Slope = (mlr.Coefficients |> Seq.head); Intercept = (mlr.Coefficients.[1]) }

let regressionResults =
    rw
    |> List.filter(fun s -> regionalD15N.Rows |> Seq.exists(fun x -> x.Shrub = s.Identifier.Value ) || yuribeiD15N.Rows |> Seq.exists(fun x -> x.``Plant Code`` = s.Identifier.Value ))
    |> List.map(fun plant ->
        let x,y = plant |> lowerResolution |> List.toArray |> Array.unzip
        (regression (x |> Array.map float) y, plant.Identifier.Value ) )
    //|> List.skip 10 |> List.take 1

regressionResults
|> List.map(fun (slope, i) ->
    printfn "%s,%f,%f,%f,%f,%f,%f,%f,%f" i slope.PValue slope.Slope.Value slope.Slope.ConfidenceLower slope.Slope.ConfidenceUpper slope.Intercept.TTest.PValue slope.Intercept.Value slope.Intercept.ConfidenceLower slope.Intercept.ConfidenceUpper )