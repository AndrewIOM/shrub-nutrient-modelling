...
module ModelComponents =
    ...
    /// Hypothesis Group A. Plant growth model
    /// Models from Pommerening et al 2015
    module GrowthFunctions =

        let e = System.Math.E

        let chapmanRichards (p:ParameterPool) t x =
            let a = p |> Parameter.Pool.getEstimate "a"
            let b = p |> Parameter.Pool.getEstimate "b"
            let c = p |> Parameter.Pool.getEstimate "c"
            a * b * c * e ** (- b * t) * (1. - e ** (-b * t)) ** (c - 1.)

        let gompertz (p:ParameterPool) t x =
            let a = p |> Parameter.Pool.getEstimate "a"
            let b = p |> Parameter.Pool.getEstimate "b"
            let c = p |> Parameter.Pool.getEstimate "c"
            a * b * c * e ** (-c * t) * e ** (- b * e ** (- c * t))

        let korf (p:ParameterPool) t x =
            let a = p |> Parameter.Pool.getEstimate "a"
            let b = p |> Parameter.Pool.getEstimate "b"
            let c = p |> Parameter.Pool.getEstimate "c"
            a * b * c * t ** (-c - 1.) * e ** (-b * e ** (-c * t))

        let monomolecular (p:ParameterPool) t x =
            let a = p |> Parameter.Pool.getEstimate "a"
            let b = p |> Parameter.Pool.getEstimate "b"
            let c = p |> Parameter.Pool.getEstimate "c"
            a * b * c * e ** (-b * t)

        let weibull (p:ParameterPool) t x =
            let a = p |> Parameter.Pool.getEstimate "a"
            let b = p |> Parameter.Pool.getEstimate "b"
            let c = p |> Parameter.Pool.getEstimate "c"
            a * b * c * t ** (c - 1.) * e ** (-b * t ** c)
    ...
    module Allocation =
        ...
        /// B1. No allocation in model.
        let none x =
            x

        /// B2. Allocation is equal between stem, roots, and leaves
        let equal x =
            x

        /// B3. Allocation between stem, roots, and leaves depends on parameters
        let variable x =
            x


    // Hypothesis Group C. Nitrogen-dependency of plant growth
    ...
    // Hypothesis Group D. Plant-Soil feedback mechanisms
    module PlantSoilFeedback =
...
let ``nutrient-dependent growth`` =
    ...
    let f = ModelComponents.GrowthLimitation.michaelisMenten
