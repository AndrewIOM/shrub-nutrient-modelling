    ...
    module Allometrics =
        ...
        let shrubRadius b a rtip p lmin k5 k6 n woodDensity mass =
            let findRadius volume =
                ...
                Optimisation.RootFinding.bisect 0 200 f 0.01 1000.00 1e-8 // Assumption that shrub radius is between 0.01 and 20.0cm.
            mass
...
let ``nutrient-dependent growth`` =
    ...
    { Equations  = [ ShortCode.create "x",      drwdt
      ...
      Parameters = [ // for growth function
                     ShortCode.create "fMax",   Parameter.create Unconstrained   0.100 0.600   // Maximum rate of resource-saturated growth per unit biomass
                     ShortCode.create "h",      Parameter.create Unconstrained   0.800 0.900   // Soil resource availability for growth at half of maximum rate
                     // for nitrogen replenishment
                     ShortCode.create "lambda", Parameter.create Unconstrained   0.010 0.025   // Rate of nitrogen replenishment
                     ShortCode.create "gamman", Parameter.create Unconstrained   0.001 0.010   // Loss rate of nitrogen
                     ...
                     ShortCode.create "q",      Parameter.create Unconstrained   0.00000001 0.0001   // Nutrient requirement per unit biomass
                     ShortCode.create "gammab", Parameter.create Unconstrained   0.001 0.100   // Loss rate of biomass
                     // for likelihood function
                     ShortCode.create "sigmax", Parameter.create PositiveOnly   0.10 0.15   // Standard deviation of x (biomass)
                     ShortCode.create "sigmay", Parameter.create PositiveOnly   0.10 0.15   // Standard deviation of y (nitrogen)
                     ShortCode.create "rho",    Parameter.create Unconstrained -0.10 0.10   // Covariance between growth and nitrogen
                    ] |> Map.ofList
...
let getStartValues startDate (plant:PlantIndividual.PlantIndividual) =
    let initialRadius =
        match plant.Growth with
        | PlantIndividual.PlantGrowth.RingWidth s -> 
            match s with
            | GrowthSeries.Absolute c -> c.Head |> fst |> PlantIndividual.removeUnit
            | GrowthSeries.Cumulative c -> 6.3 //c.Head |> fst |> removeUnit
            | _ -> invalidOp "Not implemented"
        | _ -> invalidOp "Not implemented 2"
    let initialMass = initialRadius |> PlantIndividual.removeUnit //|> toBiomass
...
let chainsDataFrame =
    ...
    |> Array.mapi (fun chainNumber chain -> 
        ...
        |> snd
        |> List.rev
        |> List.skip 5000 // Skip burn in period
